...
- **Deferred scroll content — measure children, resolve at `end`**: A scroll area does not require its content size up front; it discovers it by measuring its children. This rests on three pieces:
  ...
  - **begin → end split.** `begin` only reserves the gutter, pushes the content clip, makes hover scroll claims (from *last frame's* extent, persisted in `ScrollState.content_size`), and returns the unbounded inner space. Everything that needs the content extent moves to `end`, which receives it via `finish()`: `max_scroll = content_extent − content_bounds`, the offset clamp, wheel/page-key application, the scrollbar thumb sizes, and the slider draw (emitted after `PopClip`, so scrollbars sit on top of and outside the content clip). Two accepted consequences: a **one-frame lag** — children lay out against the offset and claims captured at `begin`, so a hard content shrink can over-scroll for a single frame — and scrollbars drawing on top of content (correct layering, and free).
