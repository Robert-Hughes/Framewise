...
- **Fit-to-Children Containers (Opt-in Sizing)**: Container widgets (such as `frame`) can choose to **opt in** to discovering their children's bounds to dynamically size themselves bottom-up. Standard single-pass leaf widgets call `layout(params, intrinsic)` to obtain their concrete bounds in one go. In contrast, container widgets that want to fit to their content size opt into the deferred layout pattern using a compile-safe token-borrow model:
  - **The Opt-In Pattern**:
    ...
    4. Sibling widgets are laid out sequentially within the child context. When the child context is finished, `finish()` automatically queries the child layout state for its accumulated `content_extent` and passes it to the `on_finish` closure.
...
- **`on_finish` in `WidgetContext`**: Every `WidgetContext` carries `on_finish: CF` where `CF: FnOnce(&mut FocusSystem, Vec2) -> DrawCommands`. Root contexts use a no-op function pointer. Container widgets construct a child via `child_with_layout_and_on_finish(layout, closure)`, passing a move closure that captures the container's token (and, for a scroll area, its `&mut ScrollState` and `&Input`). `finish()` reads the child layout's `content_extent`, calls the closure with it, and appends the returned post-commands (e.g. `PopClip`, scrollbars, focus claims) into the shared buffer after the child's own accumulated commands.

- **Deferred scroll content — measure children, resolve at `end`**: A scroll area does not require its content size up front; it discovers it by measuring its children. This rests on three pieces:
  - **Reserve policy.** Scrollbar visibility is `None` or `Always` (no `Auto`). An enabled scrollbar *always* reserves its gutter, even when the content turns out to fit. This makes `content_bounds` (the clipped viewport) known at `begin` from the rect alone, independent of the content height — removing the steals-width feedback loop where the scrollbar's presence depended on content that depended on the width.
  - **Unbounded scrollable axis.** `begin` hands children a `LayoutSpace` whose scrolling axis is `Unbounded`, so content extends past the viewport; the layout's cursor stays concrete and its `content_extent` is the measured size.
  - **begin → end split.** `begin` only reserves the gutter, pushes the content clip, and returns the unbounded inner space. Everything that needs the content extent moves to `end`, which receives it via `finish()`: `max_scroll = content_extent − content_bounds`, the offset clamp, hover scroll claims (using this frame's true content extent via first-caller-wins at `end()`), wheel/page-key application, the scrollbar thumb sizes, and the slider draw (emitted after `PopClip`, so scrollbars sit on top of and outside the content clip). The **one-frame lag** applies only to children laying out against the offset captured at `begin` (e.g. a hard content shrink can over-scroll for a single frame), but scroll claims are always decided using correct, non-stale content boundaries.
