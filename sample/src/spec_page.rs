...
use framewise::{
    ...
    widgets::{
        ...
        frame::{FrameSpec, FrameStyle, frame},
...
pub struct SpecPageState {
    ...
    // 04 Sliders
    pub slider1_state: SliderState, pub slider1_val: f32,
    pub slider2_state: SliderState, pub slider2_val: f32,
    pub slider3_state: SliderState, pub slider3_val: f32,
    pub slider4_state: SliderState, pub slider4_val: f32, // stepped 0–9
...
impl Default for SpecPageState {
    fn default() -> Self {
        ...
        Self {
            ...
            slider1_state: SliderState::default(), slider1_val: 0.14,
            slider2_state: SliderState::default(), slider2_val: 0.62,
            slider3_state: SliderState::default(), slider3_val: 0.88,
            slider4_state: SliderState::default(), slider4_val: 3.0,
            scroll_vert:  ScrollState::default(),
            scroll_horiz: ScrollState::default(),
            scroll_both:  ScrollState::default(),
            iu_fps_slider: SliderState::default(), iu_fps_val: 60.0,
...
fn sec_y<S: LayoutState<Params = Rect>>(
    ...
    t: &Theme, lx: f32, y: f32, w: f32, num: &str, title: &str,
) {
    b.divider(Rect::new(lx, y, w, 36.0));
    b.label_styled(Rect::new(lx, y, 40.0, 20.0), num, t.text_sm, t.muted, false);
    b.label_styled(Rect::new(lx + 44.0, y, w - 44.0, 22.0), title, 18.0, t.ink, false);
}

fn group_y<S: LayoutState<Params = Rect>>(
    b: &mut Builder<SampleTextSystem, S>,
    t: &Theme, lx: f32, y: f32, text: &str,
) {
    b.label_styled(Rect::new(lx, y, 400.0, 16.0), text, t.text_sm, t.muted, false);
}
...
pub fn draw_spec_page(
    ...
    let bg = frame(FrameSpec {
        rect: win_rect,
        style: FrameStyle { background: t.paper, border: t.paper, border_width: 0.0, padding: 0.0 },
    });
    ...
    // ── HERO ─────────────────────────────────────────────────────────────────
    {
        ...
        b.append_cmds(vec![
            // left bracket
            DrawCmd::StrokeLine { p0: Vec2::new(lx0+56.*ls,ly0+40.*ls), p1: Vec2::new(lx0+40.*ls,ly0+40.*ls), color: t.ink, width: lw },
            DrawCmd::StrokeLine { p0: Vec2::new(lx0+40.*ls,ly0+40.*ls), p1: Vec2::new(lx0+40.*ls,ly0+160.*ls), color: t.ink, width: lw },
            DrawCmd::StrokeLine { p0: Vec2::new(lx0+40.*ls,ly0+160.*ls), p1: Vec2::new(lx0+56.*ls,ly0+160.*ls), color: t.ink, width: lw },
            // top horizontal
            DrawCmd::StrokeLine { p0: Vec2::new(lx0+78.*ls,ly0+40.*ls), p1: Vec2::new(lx0+140.*ls,ly0+40.*ls), color: t.ink, width: lw },
            // middle horizontal (rust)
            DrawCmd::StrokeLine { p0: Vec2::new(lx0+78.*ls,ly0+96.*ls), p1: Vec2::new(lx0+120.*ls,ly0+96.*ls), color: t.rust, width: lw },
            // vertical
            DrawCmd::StrokeLine { p0: Vec2::new(lx0+78.*ls,ly0+40.*ls), p1: Vec2::new(lx0+78.*ls,ly0+160.*ls), color: t.ink, width: lw },
        ]);

        let tx = lx + 116.0;
        b.label_styled(Rect::new(tx, MARGIN, content_w - 116.0, 16.0),
            "framewise · widget specification · v0.1", t.text_sm, t.muted, false);
        b.label_styled(Rect::new(tx, MARGIN + 20.0, content_w - 116.0, 36.0),
            "A widget set that explains itself.", 28.0, t.ink, false);
        ...
        let meta_items: &[(&str, &str)] = &[
            ("ink", "#15130f"), ("paper", "#f4f1ea"), ("rust", "#c25a2c"),
            ("type", "Inter Tight · JetBrains Mono"),
        ];
        let mut mx = tx;
        for (key, val) in meta_items {
            b.label_styled(Rect::new(mx, MARGIN + 96.0, 20.0, 14.0), key, t.text_sm, t.ink, false);
            let key_w = key.len() as f32 * 7.5 + 4.0;
            b.label_styled(Rect::new(mx + key_w, MARGIN + 96.0, 200.0, 14.0), val, t.text_sm, t.muted, false);
    ...
    y += 20.0;
    {
        ...
        let row_styles: &[ButtonStyle] = &[
            ButtonStyle::default(), ButtonStyle::primary(),
            ButtonStyle::accent(),  ButtonStyle::ghost(),
        ];
        ...
        for (ci, col) in col_labels.iter().enumerate() {
            b.label_styled(
                ...
                col, t.text_sm, t.muted, false,
            );
        }
        y += 20.0;

        for (ri, row_label) in row_labels.iter().enumerate() {
            b.label_styled(Rect::new(lx, y, label_w - 8.0, t.h_md), row_label, t.text_sm, t.muted, false);
            for ci in 0..5 {
                let idx = ri * 5 + ci;
                let disabled = ci == 4;
                let btn = b.button_styled(
                    ...
                    "Action", row_styles[ri].clone(), disabled, input,
                );
    ...
    y += 20.0;
    {
        ...
        // draw group border
        let grp1_x = bx;
        let grp1_w: f32 = grp1.iter().map(|(l, _)| l.len() as f32 * 7.0 + 20.0).sum::<f32>();
        ...
        let grp2_x = bx;
        let grp2_w: f32 = grp2.iter().map(|(l, _)| l.len() as f32 * 7.0 + 20.0).sum::<f32>();
    ...
    y += 20.0;
    {
        ...
        for (ri, row_label) in row_labels.iter().enumerate() {
            b.label_styled(Rect::new(lx, y, label_w - 4.0, t.h_md), row_label, t.text_sm, t.muted, false);
            for ci in 0..5 {
                let idx = ri * 5 + ci;
                let error    = ci == 3;
                let disabled = ci == 4;
    ...
    y += 20.0;
    {
        // Labelled field
        let field_x = lx;
        b.label_styled(Rect::new(field_x, y, 120.0, 14.0), "crate name", t.text_sm, t.muted, false);
        let info = b.text_edit_ext(
        ...
        b.label_styled(Rect::new(field_x, y + 18.0 + t.h_md + 4.0, 200.0, 14.0),
            "a–z, 0–9, hyphen; max 64", t.text_sm, t.muted, false);

        // Prefixed field (draw prefix addon manually)
        let pf_x = lx + 200.0;
        b.label_styled(Rect::new(pf_x, y, 120.0, 14.0), "version", t.text_sm, t.muted, false);
        b.append_cmds(vec![
            DrawCmd::FillRect  { rect: Rect::new(pf_x, y + 18.0, 24.0, t.h_md), color: t.hover },
            DrawCmd::StrokeRect{ rect: Rect::new(pf_x, y + 18.0, 24.0, t.h_md), color: t.line, width: 1.0 },
        ]);
        b.label_styled(Rect::new(pf_x + 6.0, y + 18.0 + 7.0, 16.0, 14.0), "v", t.text_sm, t.muted, false);
        let info = b.text_edit_ext(
        ...
        b.label_styled(Rect::new(pf_x, y + 18.0 + t.h_md + 4.0, 200.0, 14.0),
            "semver mismatch — bump minor", t.text_sm, t.rust, false);

        // Multiline field
        let ml_x = lx + 420.0;
        b.label_styled(Rect::new(ml_x, y, 120.0, 14.0), "description", t.text_sm, t.muted, false);
        let info = b.text_edit_ext(
    ...
    sec_y(&mut b, &t, lx, y, content_w, "03", "Checkboxes, radios & switches");
    y += 46.0;
    ...
    y += 20.0;
    {
        ...
        // Row 1: box only
        b.label_styled(Rect::new(lx, y, label_w - 4.0, 14.0), "box", t.text_sm, t.muted, false);
        ...
        for (ci, (cs, focused, disabled)) in box_specs.iter().enumerate() {
            let dc = checkbox(CheckboxSpec {
                ...
                state: *cs, focused: *focused, disabled: *disabled,
            });
            b.append_cmds(dc.0);
        }
        y += 14.0 + 12.0;
        ...
        // Row 2: with label
        b.label_styled(Rect::new(lx, y, label_w - 4.0, 14.0), "with label", t.text_sm, t.muted, false);
        for (ci, (cs, focused, disabled)) in box_specs.iter().enumerate() {
            ...
            b.label_styled(Rect::new(cx + 18.0, y, 60.0, 14.0), "vsync", t.text_sm, label_alpha, false);
        }
        y += 14.0;
    }
    y += GROUP_GAP;
    ...
    y += 20.0;
    {
        ...
        for (i, (selected, focused, disabled, label)) in radio_items.iter().enumerate() {
            ...
            b.append_cmds(dc.0);
            b.label_styled(Rect::new(lx + 18.0, ry, 140.0, 14.0), label, t.text_md, t.ink, false);
        }
        let sw_x = lx + 220.0;
        for (i, (on, focused, disabled, label)) in switch_items.iter().enumerate() {
            ...
            b.label_styled(Rect::new(sw_x + 36.0, ry, 140.0, 16.0), label, t.text_md, label_color, false);
        }
    }
    y += 4.0 * 22.0 + SEC_GAP;

    // ── 04 · SLIDERS · DRAGS ─────────────────────────────────────────────────
    sec_y(&mut b, &t, lx, y, content_w, "04", "Sliders & numeric drags");
    y += 46.0;
    ...
    y += 20.0;
    {
        ...
        b.slider(&mut state.slider1_state, &mut state.slider1_val,
            0.0, 1.0, 0.1, SliderOrientation::Horizontal,
            Rect::new(lx, y, slider_w, t.h_md), input);
        b.label_styled(Rect::new(lx + slider_w + 12.0, y + 6.0, 80.0, 14.0),
            &format!("{:.2}", state.slider1_val), t.text_sm, t.ink, false);
        y += t.h_md + row_gap;

        b.slider(&mut state.slider2_state, &mut state.slider2_val,
            0.0, 1.0, 0.1, SliderOrientation::Horizontal,
            Rect::new(lx, y, slider_w, t.h_md), input);
        b.label_styled(Rect::new(lx + slider_w + 12.0, y + 6.0, 80.0, 14.0),
            &format!("{:.2}", state.slider2_val), t.text_sm, t.ink, false);
        y += t.h_md + row_gap;

        b.slider(&mut state.slider3_state, &mut state.slider3_val,
            0.0, 1.0, 0.1, SliderOrientation::Horizontal,
            Rect::new(lx, y, slider_w, t.h_md), input);
        b.label_styled(Rect::new(lx + slider_w + 12.0, y + 6.0, 80.0, 14.0),
            &format!("{:.2}", state.slider3_val), t.text_sm, t.ink, false);
        y += t.h_md + row_gap;

        // Stepped slider (0–9) with tick marks
        b.slider(&mut state.slider4_state, &mut state.slider4_val,
            0.0, 9.0, 1.0, SliderOrientation::Horizontal,
            Rect::new(lx, y, slider_w, t.h_md), input);
        b.label_styled(Rect::new(lx + slider_w + 12.0, y + 6.0, 80.0, 14.0),
            &format!("{:.0} / 9", state.slider4_val), t.text_sm, t.ink, false);
    ...
    y += 20.0;
    {
        ...
        b.append_cmds(vec![
            // full track
            DrawCmd::FillRect { rect: Rect::new(lx, mid_y - 0.75, track_w, 1.5), color: t.line },
            // fill bar
            DrawCmd::FillRect { rect: Rect::new(fill_x1, mid_y - 0.75, fill_x2 - fill_x1, 1.5), color: t.ink },
            // thumb 1
            DrawCmd::FillRect  { rect: Rect::new(fill_x1 - half_ts, mid_y - half_ts, ts, ts), color: t.paper_elev },
            DrawCmd::StrokeRect{ rect: Rect::new(fill_x1 - half_ts, mid_y - half_ts, ts, ts), color: t.ink, width: 1.5 },
            // thumb 2
            DrawCmd::FillRect  { rect: Rect::new(fill_x2 - half_ts, mid_y - half_ts, ts, ts), color: t.paper_elev },
            DrawCmd::StrokeRect{ rect: Rect::new(fill_x2 - half_ts, mid_y - half_ts, ts, ts), color: t.ink, width: 1.5 },
        ]);
        b.label_styled(Rect::new(lx + track_w + 12.0, y + 6.0, 80.0, 14.0),
            ".24–.76", t.text_sm, t.ink, false);
    }
    y += t.h_md + GROUP_GAP;
    ...
    y += 20.0;
    {
        ...
        let stepper_x = lx;
        b.append_cmds(vec![
            DrawCmd::FillRect  { rect: Rect::new(stepper_x, y, 64.0, t.h_md), color: t.hover },
            DrawCmd::StrokeRect{ rect: Rect::new(stepper_x, y, 64.0, t.h_md), color: t.line, width: 1.0 },
        ]);
        b.label_styled(Rect::new(stepper_x + 6.0, y + 7.0, 56.0, 14.0), "padding", t.text_sm, t.muted, false);
        b.append_cmds(vec![
            DrawCmd::FillRect  { rect: Rect::new(stepper_x + 64.0, y, 40.0, t.h_md), color: t.paper_elev },
            DrawCmd::StrokeRect{ rect: Rect::new(stepper_x + 64.0, y, 40.0, t.h_md), color: t.line, width: 1.0 },
        ]);
        b.label_styled(Rect::new(stepper_x + 72.0, y + 7.0, 24.0, 14.0), "12", t.text_sm, t.ink, false);

        // +/- buttons as text
        let sx = stepper_x + 120.0;
        b.append_cmds(vec![
            DrawCmd::FillRect  { rect: Rect::new(sx,       y, 22.0, t.h_sm), color: t.paper_elev },
            DrawCmd::StrokeRect{ rect: Rect::new(sx,       y, 22.0, t.h_sm), color: t.line, width: 1.0 },
            DrawCmd::FillRect  { rect: Rect::new(sx + 22., y, 40.0, t.h_sm), color: t.paper_elev },
            DrawCmd::StrokeRect{ rect: Rect::new(sx + 22., y, 40.0, t.h_sm), color: t.line, width: 1.0 },
            DrawCmd::FillRect  { rect: Rect::new(sx + 62., y, 22.0, t.h_sm), color: t.paper_elev },
            DrawCmd::StrokeRect{ rect: Rect::new(sx + 62., y, 22.0, t.h_sm), color: t.line, width: 1.0 },
        ]);
        b.label_styled(Rect::new(sx + 6.0,  y + 4.0, 10.0, 14.0), "−", t.text_sm, t.ink, false);
        b.label_styled(Rect::new(sx + 28.0, y + 4.0, 28.0, 14.0), "12",  t.text_sm, t.ink, false);
        b.label_styled(Rect::new(sx + 68.0, y + 4.0, 10.0, 14.0), "+", t.text_sm, t.ink, false);
        ...
        for (color, hex) in swatches {
            let dc = color_swatch(ColorSwatchSpec {
                ...
                color: *color, border: t.line,
            });
            b.append_cmds(dc.draw.0);
            b.label_styled(Rect::new(bx + 22.0, y + 7.0, 60.0, 14.0), hex, t.text_sm, t.ink, false);
            bx += 86.0;
        }
    }
    y += t.h_md + SEC_GAP;
    ...
    y += 20.0;
    {
        ...
        let dc = select(SelectSpec {
            ...
            value: "Layout: row", options: &opts, open: false, focused: false, hovered: None,
        }, b.text_system);
        b.append_cmds(dc.0);
        let dc = select(SelectSpec {
            rect: Rect::new(lx, y + t.h_md + 4.0, 160.0, t.h_md),
            value: "Layout: row", options: &opts, open: true, focused: true, hovered: Some(0),
        }, b.text_system);
        b.append_cmds(dc.0);
        ...
        let dc = segmented(SegmentedSpec {
            ...
            items: &segs1, active_index: 0, focused: None,
        }, b.text_system);
        b.append_cmds(dc.0);
        let segs2 = ["start", "center", "end"];
        let dc = segmented(SegmentedSpec {
            rect: Rect::new(seg_x, y + t.h_md + 4.0, 0.0, t.h_md),
            items: &segs2, active_index: 1, focused: None,
        }, b.text_system);
        b.append_cmds(dc.0);

        // Chips
        let chip_data: &[(&str, bool)] = &[
            ("opengl", true), ("vulkan", false), ("metal", false), ("wgpu", false),
        ];
        let chip_y = y;
        ...
        for (label, active) in chip_data {
            ...
            let chip_w = dc.0.iter().fold(0.0_f32, |acc, cmd| match cmd {
                DrawCmd::FillRect { rect, .. } | DrawCmd::StrokeRect { rect, .. } => acc.max(rect.x + rect.w - chip_x),
                _ => acc,
            });
            b.append_cmds(dc.0);
            chip_x += chip_w + 6.0;
        ...
        let dc = chip(ChipSpec {
            ...
            label: "+ add backend", active: false, focused: false,
        }, b.text_system);
        b.append_cmds(dc.0);
    }
    ...
    y += 20.0;
    {
        let items1 = [
            ...
            MenuItem::Item { label: "New panel",       shortcut: Some("⌘ N"),   selected: false, disabled: false },
            MenuItem::Item { label: "Duplicate",       shortcut: Some("⌘ D"),   selected: true,  disabled: false },
            MenuItem::Item { label: "Detach",          shortcut: Some("⌘ ⇧ D"), selected: false, disabled: false },
            MenuItem::Separator,
            MenuItem::Group("Inspect"),
            MenuItem::Item { label: "Show layout grid",shortcut: Some("G"),     selected: false, disabled: false },
            MenuItem::Item { label: "Show id tree",    shortcut: Some("⌘ ⇧ I"),selected: false, disabled: false },
            MenuItem::Separator,
            MenuItem::Item { label: "Replay last frame",shortcut:Some("F2"),   selected: false, disabled: true  },
        ];
        let dc = menu(MenuSpec { rect: Rect::new(lx, y, 240.0, 0.0), items: &items1 }, b.text_system);
        b.append_cmds(dc.0);

        let items2 = [
            MenuItem::Group("Theme"),
            MenuItem::Item { label: "framewise · default", shortcut: None, selected: false, disabled: false },
            MenuItem::Item { label: "framewise · ink",     shortcut: None, selected: true,  disabled: false },
            MenuItem::Item { label: "framewise · paper",   shortcut: None, selected: false, disabled: false },
            MenuItem::Item { label: "custom…",             shortcut: None, selected: false, disabled: false },
        ];
        let dc = menu(MenuSpec { rect: Rect::new(lx + 264.0, y, 200.0, 0.0), items: &items2 }, b.text_system);
        b.append_cmds(dc.0);

        let menu1_h: f32 = items1.iter().map(|i| match i {
            MenuItem::Item{..} => 26.0, MenuItem::Separator => 9.0, MenuItem::Group(_) => 22.0,
        }).sum::<f32>() + 8.0;
        y += menu1_h;
    }
    y += SEC_GAP;
    ...
    y += 46.0;
    {
        ...
        b.append_cmds(vec![DrawCmd::StrokeRect { rect: b1, color: t.line, width: 1.0 }]);
        {
            let (pre, scope, cb, _) = begin_scroll_area(
                b1, b1_content, ScrollbarVisibility::None, ScrollbarVisibility::Always,
                &mut state.scroll_vert, ManualLayout, input, b.focus_sys, None, time,
            );
            b.append_cmds(pre);
            let code_lines = [
            ...
            for (i, line) in code_lines.iter().enumerate() {
                b.label_styled(
                    ...
                    line, t.text_sm, t.muted, false,
                );
            }
            let post = scope.finish(b.focus_sys);
            b.append_cmds(post);
        }
        b.label_styled(Rect::new(b1.x, y + b1.h + 4.0, b1.w, cap_h),
            "vertical · idle", t.text_sm, t.muted, false);
        ...
        b.append_cmds(vec![DrawCmd::StrokeRect { rect: b2, color: t.line, width: 1.0 }]);
        {
            let (pre, scope, cb, _) = begin_scroll_area(
                b2, b2_content, ScrollbarVisibility::None, ScrollbarVisibility::Always,
                &mut state.scroll_horiz, ManualLayout, input, b.focus_sys, None, time,
            );
            b.append_cmds(pre);
        ...
        b.label_styled(Rect::new(b2.x, y + b2.h + 4.0, b2.w, cap_h),
            "vertical · dragging (rust)", t.text_sm, t.muted, false);

        // Box 3: horizontal
        let b3_x = b2_x + b2.w + box_gap;
        let b3 = Rect::new(b3_x, y + 15.0, 300.0, 100.0);
        let b3_content = Vec2::new(700.0, 100.0);
        b.append_cmds(vec![DrawCmd::StrokeRect { rect: b3, color: t.line, width: 1.0 }]);
        {
            let (pre, scope, cb, _) = begin_scroll_area(
                b3, b3_content, ScrollbarVisibility::Always, ScrollbarVisibility::None,
                &mut state.scroll_both, ManualLayout, input, b.focus_sys, None, time,
            );
            b.append_cmds(pre);
        ...
        b.label_styled(Rect::new(b3.x, y + b3.h + 15.0 + 4.0, b3.w, cap_h),
            "horizontal", t.text_sm, t.muted, false);
    ...
    y += 46.0;
    {
        ...
        let dc = tabs(TabsSpec {
            ...
            items: &tabs1, active_index: 0, focused: None,
        }, b.text_system);
        b.append_cmds(dc.0);
        y += 36.0 + 20.0;
        ...
        let dc = tabs(TabsSpec {
            ...
            items: &tabs2, active_index: 1, focused: None,
        }, b.text_system);
        b.append_cmds(dc.0);
        y += 36.0;
    }
    ...
    sec_y(&mut b, &t, lx, y, content_w, "08", "Progress, meters & status");
    y += 46.0;
    ...
    y += 20.0;
    {
        ...
        for (val, active, label) in bar_items {
            let dc = progress_bar(ProgressBarSpec {
                ...
                value: *val, phase: (time as f32) * 0.5, active: *active,
            });
            b.append_cmds(dc.0);
            b.label_styled(Rect::new(lx + bar_w + 12.0, y + 2.0, 180.0, 14.0), label, t.text_sm, t.muted, false);
            y += 22.0;
        }
    }
    y += GROUP_GAP;
    ...
    y += 20.0;
    {
        ...
        for (label, val, peak) in meters {
            b.label_styled(Rect::new(bx, y, 36.0, 14.0), label, t.text_sm, t.muted, false);
            bx += 40.0;
            if *label == "FRAME" {
                b.label_styled(Rect::new(bx, y - 1.0, 60.0, 16.0), "2.4 ms", t.text_sm, t.ink, false);
                bx += 70.0;
            } else {
                let dc = meter(MeterSpec {
                    rect: Rect::new(bx, y, 100.0, 12.0),
                    value: *val, peak: *peak, bars: 10,
                });
                b.append_cmds(dc.draw.0);
                bx += 108.0;
            }
    ...
    y += 20.0;
    {
        let dc = spinner(SpinnerSpec { rect: Rect::new(lx, y, 16.0, 16.0), large: false, color: None });
        b.append_cmds(dc.0);
        b.label_styled(Rect::new(lx + 20.0, y + 1.0, 60.0, 14.0), "loading", t.text_sm, t.muted, false);

        let dc = spinner(SpinnerSpec { rect: Rect::new(lx + 90.0, y - 4.0, 24.0, 24.0), large: true, color: None });
        b.append_cmds(dc.0);
        b.label_styled(Rect::new(lx + 118.0, y + 1.0, 50.0, 14.0), "large", t.text_sm, t.muted, false);
    ...
    {
        let widget_tree = [
            TreeRow { indent: 0, caret: Some(true),  label: "App",        meta: Some("#0001"), selected: false },
            TreeRow { indent: 1, caret: Some(true),  label: "MenuBar",    meta: Some("#0002"), selected: false },
            TreeRow { indent: 2, caret: None,         label: "File",      meta: Some("#0003"), selected: false },
            TreeRow { indent: 2, caret: None,         label: "Edit",      meta: Some("#0004"), selected: false },
            TreeRow { indent: 1, caret: Some(true),  label: "Workspace",  meta: Some("#0010"), selected: false },
            TreeRow { indent: 2, caret: Some(true),  label: "Canvas",     meta: Some("#0011"), selected: false },
            TreeRow { indent: 3, caret: None,         label: "Layer \"frame\"", meta: Some("#0014"), selected: true  },
            TreeRow { indent: 3, caret: None,         label: "Layer \"ui\"",    meta: Some("#0015"), selected: false },
            TreeRow { indent: 2, caret: Some(false), label: "Inspector",  meta: Some("#0020"), selected: false },
            TreeRow { indent: 1, caret: Some(false), label: "StatusBar",  meta: Some("#0030"), selected: false },
        ];
        let dc = tree(TreeSpec { rect: Rect::new(lx, y, 320.0, 0.0), rows: &widget_tree }, b.text_system);
        b.append_cmds(dc.0);

        let file_list = [
            TreeRow { indent: 0, caret: None, label: "frame_buffer.rs", meta: Some("2.1 kb"), selected: false },
            TreeRow { indent: 0, caret: None, label: "layout.rs",       meta: Some("5.4 kb"), selected: false },
            TreeRow { indent: 0, caret: None, label: "renderer.rs",     meta: Some("12.0 kb"),selected: true  },
            TreeRow { indent: 0, caret: None, label: "state.rs",        meta: Some("3.8 kb"), selected: false },
            TreeRow { indent: 0, caret: None, label: "theme.rs",        meta: Some("1.6 kb"), selected: false },
            TreeRow { indent: 0, caret: None, label: "widget/",         meta: Some("11 files"),selected: false},
            TreeRow { indent: 0, caret: None, label: "main.rs",         meta: Some("0.4 kb"), selected: false },
        ];
        let dc = tree(TreeSpec { rect: Rect::new(lx + 360.0, y, 240.0, 0.0), rows: &file_list }, b.text_system);
        b.append_cmds(dc.0);
    ...
    y += 20.0;
    {
        ...
        for (keys, desc) in key_rows {
            ...
            b.label_styled(Rect::new(kx + 4.0, y + 3.0, 200.0, 14.0), desc, t.text_sm, t.muted, false);
            y += 28.0;
        }
    }
    y += SEC_GAP;
    ...
    y += 46.0;
    {
        ...
        let win_res = window(WindowSpec {
            rect: win_rect, title: "Inspector",
            buttons: &win_buttons, status_bar: true,
        ...
        for (label, val, min, max) in drag_items {
            let dc = drag_number(DragNumberSpec {
                ...
                label, value: *val, min: *min, max: *max, active: false,
            }, b.text_system);
            b.append_cmds(dc.0);
            drx += (cr.w / 2.0) + 4.0;
        }
        ...
        for (cs, label) in check_items {
            let dc = checkbox(CheckboxSpec {
                ...
                state: *cs, focused: false, disabled: false,
            });
            b.append_cmds(dc.0);
            b.label_styled(Rect::new(cr.x + 18.0, iy, cr.w - 18.0, 14.0), label, t.text_md, t.ink, false);
            iy += 22.0;
        }
        ...
        b.append_cmds(vec![
            DrawCmd::FillRect  { rect: dw, color: dark_bg },
            DrawCmd::StrokeRect{ rect: dw, color: dark_bdr, width: 1.0 },
            DrawCmd::FillRect  { rect: Rect::new(dw.x, dw.y, dw.w, 26.0), color: darker },
        ]);
        b.label_styled(Rect::new(dw.x + 10.0, y + 6.0, 180.0, 14.0),
            "framewise · dark", t.text_sm, light, false);
        b.label_styled(Rect::new(dw.x + dw.w - 28.0, y + 6.0, 20.0, 14.0), "✕", t.text_sm, light, false);

        let cx = dw.x + 16.0;
        let cyw = y + 26.0 + 16.0;
        // keycap row
        b.append_cmds(vec![
            DrawCmd::FillRect  { rect: Rect::new(cx,      cyw, 24.0, 22.0), color: Color::from_srgb_u8(42,37,32,255) },
            DrawCmd::StrokeRect{ rect: Rect::new(cx,      cyw, 24.0, 22.0), color: dark_bdr, width: 1.0 },
            DrawCmd::FillRect  { rect: Rect::new(cx+28.0, cyw, 22.0, 22.0), color: Color::from_srgb_u8(42,37,32,255) },
            DrawCmd::StrokeRect{ rect: Rect::new(cx+28.0, cyw, 22.0, 22.0), color: dark_bdr, width: 1.0 },
        ]);
        b.label_styled(Rect::new(cx + 7.0, cyw + 5.0, 12.0, 12.0), "⌘", t.text_sm, light, false);
        b.label_styled(Rect::new(cx + 35.0, cyw + 5.0, 12.0, 12.0), "K", t.text_sm, light, false);
        b.label_styled(Rect::new(cx + 56.0, cyw + 5.0, 140.0, 12.0), "search everything", t.text_sm, muted_l, false);

        // fake dark input
        let inp_y = cyw + 28.0;
        b.append_cmds(vec![
            DrawCmd::FillRect  { rect: Rect::new(cx, inp_y, dw.w - 32.0, 26.0), color: darker },
            DrawCmd::StrokeRect{ rect: Rect::new(cx, inp_y, dw.w - 32.0, 26.0), color: dark_bdr, width: 1.0 },
        ]);
        b.label_styled(Rect::new(cx + 8.0, inp_y + 7.0, dw.w - 48.0, 12.0), "type a command…", t.text_sm, muted_l, false);

        // fake dark tabs
        let tab_y = inp_y + 30.0;
        b.append_cmds(vec![DrawCmd::StrokeLine {
            p0: Vec2::new(cx, tab_y + 26.0), p1: Vec2::new(dw.x + dw.w - 16.0, tab_y + 26.0),
            color: dark_bdr, width: 1.0,
        }]);
        let tab_items = ["Files", "Symbols", "Frames"];
        let mut tab_x = cx;
        for (i, item) in tab_items.iter().enumerate() {
            b.label_styled(Rect::new(tab_x, tab_y + 5.0, 60.0, 14.0), item, t.text_sm,
                if i == 0 { light } else { muted_l }, false);
            if i == 0 {
                b.append_cmds(vec![DrawCmd::FillRect {
                    rect: Rect::new(tab_x, tab_y + 24.0, 40.0, 2.0), color: t.rust,
                }]);
            }
            tab_x += 60.0;
        }
        let file_y = tab_y + 32.0;
        for (i, file) in ["▸ renderer.rs", "▸ layout.rs", "▸ widget/button.rs"].iter().enumerate() {
            b.label_styled(Rect::new(cx, file_y + i as f32 * 18.0, 200.0, 14.0), file, t.text_sm, muted_l, false);
        }
    ...
    y += 46.0;
    {
        ...
        let win_res = window(WindowSpec {
            rect: wr, title: "Renderer Settings",
            buttons: &win_buttons, status_bar: true,
        ...
        let dc = tabs(TabsSpec {
            ...
            items: &tabs_items, active_index: 0, focused: None,
        }, b.text_system);
        b.append_cmds(dc.0);
        ...
        // backend (segmented)
        b.label_styled(Rect::new(cr.x, fy + 7.0, label_w, 14.0), "backend", t.text_sm, t.muted, false);
        let backends = ["OpenGL", "Vulkan", "Metal", "wgpu"];
        let dc = segmented(SegmentedSpec {
            rect: Rect::new(widget_x, fy, 0.0, row_h),
            items: &backends, active_index: 1, focused: None,
        }, b.text_system);
        b.append_cmds(dc.0);
        fy += row_h + row_gap;

        // target fps (slider)
        b.label_styled(Rect::new(cr.x, fy + 7.0, label_w, 14.0), "target fps", t.text_sm, t.muted, false);
        b.slider(&mut state.iu_fps_slider, &mut state.iu_fps_val,
            24.0, 240.0, 10.0, SliderOrientation::Horizontal,
            Rect::new(widget_x, fy, widget_w - 40.0, row_h), input);
        b.label_styled(Rect::new(widget_x + widget_w - 34.0, fy + 7.0, 34.0, 14.0),
            &format!("{:.0}", state.iu_fps_val), t.text_sm, t.ink, false);
        fy += row_h + row_gap;

        // vsync (switch)
        b.label_styled(Rect::new(cr.x, fy + 7.0, label_w, 14.0), "vsync", t.text_sm, t.muted, false);
        let dc = switch(SwitchSpec {
            rect: Rect::new(widget_x, fy + 6.0, 30.0, 16.0),
            on: true, focused: false, disabled: false,
        });
        b.append_cmds(dc.0);
        b.label_styled(Rect::new(widget_x + 36.0, fy + 7.0, 120.0, 14.0), "match display", t.text_sm, t.ink, false);
        fy += row_h + row_gap;

        // msaa (segmented)
        b.label_styled(Rect::new(cr.x, fy + 7.0, label_w, 14.0), "msaa", t.text_sm, t.muted, false);
        let msaa_opts = ["off", "2×", "4×", "8×"];
        let dc = segmented(SegmentedSpec {
            rect: Rect::new(widget_x, fy, 0.0, row_h),
            items: &msaa_opts, active_index: 2, focused: None,
        }, b.text_system);
        b.append_cmds(dc.0);
        fy += row_h + row_gap;

        // viewport (drag numbers)
        b.label_styled(Rect::new(cr.x, fy + 7.0, label_w, 14.0), "viewport", t.text_sm, t.muted, false);
        ...
        for (label, val) in vp_items {
            let dc = drag_number(DragNumberSpec {
                ...
                label, value: *val, min: 0.0, max: 7680.0, active: false,
            }, b.text_system);
            b.append_cmds(dc.0);
        ...
        b.label_styled(Rect::new(cr.x, fy + 7.0, label_w, 14.0), "accent", t.text_sm, t.muted, false);
        let dc = color_swatch(ColorSwatchSpec {
            rect: Rect::new(widget_x, fy + 4.0, 18.0, 20.0),
            color: t.rust, border: t.line,
        });
        b.append_cmds(dc.draw.0);
        b.label_styled(Rect::new(widget_x + 22.0, fy + 7.0, 60.0, 14.0), "#c25a2c", t.text_sm, t.ink, false);
        fy += row_h + row_gap;

        // options (checkboxes)
        b.label_styled(Rect::new(cr.x, fy + 7.0, label_w, 14.0), "options", t.text_sm, t.muted, false);
        ...
        for (i, (cs, label)) in opt_items.iter().enumerate() {
            ...
            b.append_cmds(dc.0);
            b.label_styled(Rect::new(widget_x + 18.0, opt_y + 4.0, widget_w - 18.0, 14.0), label, t.text_md, t.ink, false);
        }
        fy += 3.0 * 22.0 + 4.0;
        ...
        let fl_res = window(WindowSpec {
            rect: fl_rect, title: "Frame Log",
            buttons: &fl_buttons, status_bar: true,
        ...
        let log_content_h = log_lines.len() as f32 * 18.0 + 8.0;
        {
            let (pre, scope, lcb, _) = begin_scroll_area(
                ...
                ManualLayout, input, b.focus_sys, None, time,
            );
            b.append_cmds(pre);
            ...
            for (i, (ts_str, msg, highlight)) in log_lines.iter().enumerate() {
                ...
                let ts_w = 100.0_f32;
                b.label_styled(Rect::new(lcb.x + 6.0, row_y, ts_w, 14.0), ts_str, t.text_sm, t.muted, false);
                let msg_color = if *highlight { t.rust } else { t.ink };
                b.label_styled(Rect::new(lcb.x + 6.0 + ts_w + 8.0, row_y, lcb.w - ts_w - 14.0, 14.0), msg, t.text_sm, msg_color, false);
            }
            let post = scope.finish(b.focus_sys);
            b.append_cmds(post);
        }
        ...
        let qa_res = window(WindowSpec {
            rect: qa_rect, title: "Quick actions",
            buttons: &qa_buttons, status_bar: false, status_text: "",
        }, b.text_system);
        let qa_cr = qa_res.layout.content_bounds;
        b.append_cmds(qa_res.draw.0);

        let qa_items = [
            MenuItem::Item { label: "Render frame",         shortcut: Some("F1"),     selected: false, disabled: false },
            MenuItem::Item { label: "Replay last frame",    shortcut: Some("F2"),     selected: true,  disabled: false },
            MenuItem::Item { label: "Show id tree",         shortcut: Some("⌘ ⇧ I"), selected: false, disabled: false },
            MenuItem::Separator,
            MenuItem::Item { label: "Dump state to clipboard", shortcut: Some("⌘ ⇧ C"), selected: false, disabled: false },
        ];
        let dc = menu(MenuSpec {
    ...
    // ── FOOTER ───────────────────────────────────────────────────────────────
    {
        ...
        for (key, val) in foot_items {
            ...
            b.label_styled(Rect::new(fx + kw, y, 220.0, 14.0), val, t.text_sm, t.muted, false);
            fx += kw + val.len() as f32 * 6.5 + 24.0;
        }
        b.label_styled(Rect::new(lx + content_w - 200.0, y, 200.0, 14.0),
            "framewise · widget specification", t.text_sm, t.ink, false);
    }
        let _ = (y, b);
